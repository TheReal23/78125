...
GameManager.prototype.addRandomTile = function () {
  if (this.grid.cellsAvailable()) {
    var value = Math.random() < 0.99 ? 5 : Math.random() < 0.5 ? 0 : 0;
    ...
    traversals.y.forEach(function (y) {
      ...
      if (tile) {
        ...
        if (next && next.value === tile.value && !next.mergedFrom || next && next.value == 0 || next && tile.value == 0) {
          ...
          if(next && next.value == 0) merged.value = tile.value / 5;
          if(next && tile.value == 0) merged.value = next.value / 5;
          ...
          if (merged.value === 78125) self.won = true;
        } else {
